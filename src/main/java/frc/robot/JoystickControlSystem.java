...
public class JoystickControlSystem {
  ...
  public static void initialize(Joystick driverJoystickInput, MecanumDriveSubsystem driveSubsystemInput, GyroSubsystem Gyro, BallSubsystem ballSubsystem, XboxController xboxControllerInput) {
    ...
    ballSubsystem.setDefaultCommand(new RunCommand(() -> ballSubsystem.spinIntake(
      Math.abs(driverJoystick.getX()*((1 - driverJoystick.getThrottle()) / 2)+0.1)), ballSubsystem));
    ...
    shootBall.whenHeld(new RunCommand(() -> ballSubsystem.spinIntake(-1)));
    
    RaiseBallsToggle controllerRaiseBallsToggle = new RaiseBallsToggle(ballSubsystem, Constants.intakeLifterTopPosition);
    ...
    putBallsDown.whenPressed(new RaiseBallsPos(ballSubsystem, Constants.intakeLifterDownPosition));
    JoystickButton xboxPutBallsDown = new JoystickButton(xboxController, 3);
    xboxPutBallsDown.whenPressed(new RaiseBallsPos(ballSubsystem, Constants.intakeLifterDownPosition));

    JoystickButton putBallsUp = new JoystickButton(driverJoystick, 4);
    putBallsUp.whenPressed(new RaiseBallsPos(ballSubsystem, Constants.intakeLifterTopPosition));
    JoystickButton xboxPutBallsUp = new JoystickButton(xboxController, 2);
    xboxPutBallsUp.whenPressed(new RaiseBallsPos(ballSubsystem, Constants.intakeLifterTopPosition));
  }
